...
**Version:** 3.4.1 (Orchestrator + Docs Edition)
...
- Document is updated via UI (`mission.document.update`)
...
### 4.5 Document Handlers

**Purpose:** Provide read/write access to project documentation files.

#### 4.5.1 List Documents

```typescript
// handlerDocumentList
async function handlerDocumentList(params: { project_id: string }): Promise<Document[]> {
  ...
  const projectPath = path.join('~/.openclaw/projects', project_id);
  const docs: Document[] = [];

  // KANBAN.md
  const kanbanPath = path.join(projectPath, 'KANBAN.md');
  if (fs.existsSync(kanbanPath)) {
    docs.push({ path: 'KANBAN.md', type: 'kanban', editable: false });
  }

  // CHRONICLE.md
  const chroniclePath = path.join(projectPath, 'CHRONICLE.md');
  if (fs.existsSync(chroniclePath)) {
    docs.push({ path: 'CHRONICLE.md', type: 'chronicle', editable: true });
  }
  ...
  if (fs.existsSync(contextPath)) {
    ...
    for (const file of contextFiles) {
      docs.push({ path: `.context/${file}`, type: 'context', editable: true });
    }
  }

  // agents/*/SOUL.md, agents/*/learned.md
  const agentsPath = path.join(projectPath, 'agents');
  if (fs.existsSync(agentsPath)) {
    const agentDirs = await fs.readdir(agentsPath);
    for (const agentId of agentDirs) {
      const soulPath = path.join(agentsPath, agentId, 'SOUL.md');
      const learnedPath = path.join(agentsPath, agentId, 'learned.md');

      if (fs.existsSync(soulPath)) {
        docs.push({ path: `agents/${agentId}/SOUL.md`, type: 'agent_soul', editable: true, agentId });
      }

      if (fs.existsSync(learnedPath)) {
        docs.push({ path: `agents/${agentId}/learned.md`, type: 'agent_learned', editable: true, agentId });
      }
    }
  }
...
```typescript
// handlerDocumentGet
async function handlerDocumentGet(params: { project_id: string; path: string }): Promise<string> {
  const { project_id, path: docPath } = params;
  ...
  // Resolve full path
  const fullPath = resolveDocumentPath(project_id, docPath);
...
#### 4.5.3 Update Document

```typescript
// handlerDocumentUpdate
async function handlerDocumentUpdate(params: {
  project_id: string;
  path: string;
  content: string;
}): Promise<void> {
  const { project_id, path: docPath, content } = params;
  ...
  const docs = await handlerDocumentList({ project_id });
  const doc = docs.find(d => d.path === docPath);
  if (!doc || !doc.editable) {
    throw new Error(`Document is not editable: ${docPath}`);
  }
...
### 8.5 Project Switcher with "All Projects" Mode

**Purpose:** Allow users to view tasks across all projects or filter to a specific project.

**Behavior:**

| View Mode | Description |
|-----------|-------------|
| **All Projects** | Shows tasks from all projects (grouped by project or flat list) |
...
<ProjectSwitcher
  ...
  projects={projects}
  onProjectChange={(id) => {
    setCurrentProjectId(id);  // null = all projects
    fetchTasks(id);         // Fetch filtered or all tasks
  }}
/>
```
...
**RPC Call (All Projects):**

```typescript
// Fetch all tasks across all projects
const allTasks = await rpcCall<Task[]>('mission.task.list', {});

// Display with project grouping or flat list
```

**RPC Call (Single Project):**

```typescript
// Fetch tasks for specific project only
const projectTasks = await rpcCall<Task[]>('mission.task.list', {
  project_id: 'vanguard'
});
```

### 8.6 Documentation Tab

**Purpose:** View and edit project documentation (markdown files) in the Mission Control UI.

**Available Documents:**

| Document | Path | Description |
|----------|------|-------------|
| **KANBAN.md** | `projects/{id}/KANBAN.md` | Human-readable Kanban state (mirrored from DB) |
| **CHRONICLE.md** | `projects/{id}/CHRONICLE.md` | Project knowledge base, decisions, technical debt |
| **.context/** | `projects/{id}/.context/*` | Fixed project rules, style guides, tech stack |
| **SOUL.md** | `projects/{id}/agents/{agent_id}/SOUL.md` | Agent persona refinements |
| **learned.md** | `projects/{id}/agents/{agent_id}/learned.md` | Distilled agent wisdom |
...
interface DocumentsTabProps {
  projectId: string;
  document: string;  // e.g., 'CHRONICLE.md', 'KANBAN.md'
...
|----------|------|-------|-------|
| KANBAN.md | ✅ | ❌ | Mirrored from DB (read-only) |
| CHRONICLE.md | ✅ | ✅ | Editable via UI |
| .context/* | ✅ | ✅ | Editable via UI |
| SOUL.md | ✅ | ⚠️ | Editable via UI (agent persona refinement) |
| learned.md | ✅ | ⚠️ | Editable via UI (knowledge editing) |

**RPC Methods:**

```typescript
// Get document content
const content = await rpcCall<string>('mission.document.get', {
  project_id: 'vanguard',
  path: 'CHRONICLE.md'
});

// Update document content
await rpcCall('mission.document.update', {
  project_id: 'vanguard',
  path: 'CHRONICLE.md',
...
const docs = await rpcCall<Document[]>('mission.document.list', {
  project_id: 'vanguard'
});
```
...
```typescript
// Use a markdown editor component (e.g., react-markdown-editor-lite)
...
When a document is updated via UI:
1. Write to file (via RPC)
2. Broadcast WebSocket event: `DOCUMENT_UPDATED`
...
**Purpose:** Switch between Kanban board and Documents tab.
...
3. **Phase 5 update:** Frontend UI enhancements
...
4. **Phase 7:** Integration Testing with Nestor
   ...
   - **NEW:** Test document read/write via UI
   - **NEW:** Verify document updates broadcast via WebSocket

5. **Phase 8:** Deployment & Monitoring
...
| **Documentation Tab** | Missing | Full markdown editor with file tree |
| **Project Filtering** | Single project only | All Projects mode + per-project filter |
| **Document RPC** | Not specified | `mission.document.*` handlers (list/get/update) |
| **Document Events** | Not specified | `DOCUMENT_UPDATED` WebSocket broadcast |
