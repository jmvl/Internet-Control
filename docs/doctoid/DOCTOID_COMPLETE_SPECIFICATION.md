...
8. [Implementation Agents](#8-implementation-agents)
...
### 9.1 Phase 1: Foundation (Parallel - 3 Agents)
...
### 9.2 Phase 2: Frontend (1 Agent)

| Agent | Tasks | Output |
|-------|-------|--------|
| **Frontend** | React app, Chart.js, WebSocket client | `frontend/` |

### 9.3 Phase 3: Integration (1 Agent)
...
device_id: null  # Auto-detect or specify device ID
refresh_interval_seconds: 10
retention_days: 30
...
| Severity | Visual | Log |
|----------|--------|-----|
| Critical | 🔴 Red badge | alerts.log |
| Warning | 🟡 Yellow badge | alerts.log |
...
|-------|-----------|----------|
| Device disconnected | `adb devices` empty | Banner + auto-reconnect every 30s |
| Multiple devices | >1 device in list | Prompt to select on startup |
| Permission denied | dumpsys fails | Show "N/A", log separately |
